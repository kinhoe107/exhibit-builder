...
test("runs the ICC-style adversarial pack through analysis and bundle assembly", async () => {
  ...
  const workbookExporter = process.env.EXHIBIT_BUILDER_NATIVE_EXCEL === "1" ? nativeWorkbookExporter : testWorkbookExporter;
  const build = await buildBundle(analysis, selected, { templates, pagination: { matchPdfPageOrder: true, prefix: "", includePrefixInIndex: false }, profileId: "review-draft", workbookExporter });
  ...
  if (process.env.EXHIBIT_BUILDER_NATIVE_EXCEL === "1") {
    const nativeWorkbookExpectations = new Map([
      ["Baseline_Programme_RevC_2025-12-05.xlsx", [/Protection design freeze/i, /Handover documentation/i, /2026-04-07/]],
  ...
  const multiVolume = await buildBundle(analysis, selected, {
    templates,
