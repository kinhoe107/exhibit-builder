...
test("rejects a legacy binary DOC template at the converter boundary", async () => {
...
test("rejects a deceptively compressed DOCX after actual expansion", async () => {
  const zip = new JSZip();
  zip.file("word/document.xml", `<w:document><w:body><w:p><w:r><w:t>${"A".repeat(51 * 1024 * 1024)}</w:t></w:r></w:p></w:body></w:document>`);
  ...
  await assert.rejects(
    renderWordTemplateHtml(new File([bytes], "Compressed_Bomb.docx")),
    /50 MB offline safety limit/,
  );
});
