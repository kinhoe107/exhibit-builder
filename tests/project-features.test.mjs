...
test("schema 7 project preserves manual exhibit provenance and source bytes", async () => {
  ...
  assert.equal(opened.snapshot.schemaVersion, 8);
  assert.deepEqual(opened.snapshot.candidates, [candidate]);
...
test("saved-project recovery overlays only hash-bound current journal deltas", async () => {
  ...
  const opened = {
    snapshot: {
      ...
      candidates: [{ id: "candidate", evidenceId: "evidence", description: "Saved description", confirmed: true }],
    },
    sources: [
  ...
  const payload = {
    ...
    candidates: [{ id: "candidate", evidenceId: "evidence", sourceSha256: currentHash, description: "Edited after save", confirmed: true }],
    finalOrder: ["new"],
    ...
    statements: [{ id: "statement", sourceId: "statement", witnessName: "Updated Witness", witnessInitials: "UW" }],
