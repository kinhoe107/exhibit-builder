...
test("email-child disposition changes are substantive and ignore stale foreign keys", async () => {
  const { createSubstantiveBuildSnapshot, compareSubstantiveBuilds, substantiveBuildCanonical, nonArrangementSubstantiveKey } = await import("../app/lib/rebuild-comparison.ts");
  const childIdentity = `${"p".repeat(64)}:1:${"c".repeat(64)}`;
  const otherIdentity = `${"p".repeat(64)}:2:${"d".repeat(64)}`;
  const analysis = {
    statementHash: "a".repeat(64),
    evidence: [{ id: "e", sha256: "b".repeat(64), emailAttachments: [{ identity: childIdentity }] }],
  };
  const pagination = { scheme: "bundle", prefix: "", suffix: "", startAt: 1, padding: 0, preliminary: "roman", countTemplates: true, position: "bottom-centre", fontSize: 8, includePrefixInIndex: false };
  const layout = { includeDividerPages: false, includeExhibitCoverPages: false, countOptionalPagesInReferences: false, volumePageLimit: 0 };
  const printed = { id: "c", included: true, confirmed: true, evidenceId: "e", description: "Notice", emailAttachmentDispositions: { [childIdentity]: "print-with-email" } };
  const leftOut = { ...printed, emailAttachmentDispositions: { [childIdentity]: "leave-out" } };
  const added = { ...printed, emailAttachmentDispositions: { [childIdentity]: "add-as-exhibit" } };
  const printedSnapshot = await createSubstantiveBuildSnapshot({ analysis, candidates: [printed], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] });
  const leftOutSnapshot = await createSubstantiveBuildSnapshot({ analysis, candidates: [leftOut], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] });
  const addedSnapshot = await createSubstantiveBuildSnapshot({ analysis, candidates: [added], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] });
  assert.deepEqual(compareSubstantiveBuilds(printedSnapshot, leftOutSnapshot).categories, ["Email attachment decisions changed"]);
  assert.deepEqual(compareSubstantiveBuilds(printedSnapshot, addedSnapshot).categories, ["Email attachment decisions changed"]);

  const staleKeys = { ...printed, emailAttachmentDispositions: { [childIdentity]: "print-with-email", [otherIdentity]: "leave-out", "not-a-child": "print-with-email" } };
  const reversedKeys = { ...printed, emailAttachmentDispositions: { [otherIdentity]: "leave-out", [childIdentity]: "print-with-email" } };
  const staleSnapshot = await createSubstantiveBuildSnapshot({ analysis, candidates: [staleKeys], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] });
  const reversedSnapshot = await createSubstantiveBuildSnapshot({ analysis, candidates: [reversedKeys], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] });
  assert.equal(compareSubstantiveBuilds(printedSnapshot, staleSnapshot).summary, "No substantive change");
  assert.equal(compareSubstantiveBuilds(printedSnapshot, reversedSnapshot).summary, "No substantive change");
  assert.equal(
    nonArrangementSubstantiveKey(substantiveBuildCanonical({ analysis, candidates: [staleKeys], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] })),
    nonArrangementSubstantiveKey(substantiveBuildCanonical({ analysis, candidates: [reversedKeys], canonicalOrder: ["c"], templates: [], layout, pagination, resolutions: [] })),
  );
});
