...
test("guided sample is instructional and detects six references including a three-reference paragraph", async () => {
  const { analyseBundleStatements, SAMPLE_EVIDENCE, SAMPLE_REQUIRED_FILES, SAMPLE_STATEMENT, SAMPLE_TEMPLATES, samplePackRelativePath } = await import("../app/lib/bundle-engine.ts");
  const root = new URL("../public/guided-sample/", import.meta.url);
  const statementUrl = new URL(samplePackRelativePath(SAMPLE_STATEMENT), root);
  ...
  assert.match(documentXml, /Open the guided sample folder/);
  assert.match(documentXml, /Run guided sample/);
  assert.match(documentXml, /Choose witness statement/);
  assert.match(documentXml, /<w:drawing>/);
  assert.ok(documentZip.file("word/media/image1.png"));
  assert.match(documentXml, /<w:numPr>/);
  assert.match(numberingXml, /<w:lvlText w:val="%1\."/);
  const decimalAbstracts = [...numberingXml.matchAll(/<w:abstractNum w:abstractNumId="(\d+)"[\s\S]*?<\/w:abstractNum>/g)]
    .filter((match) => /<w:lvlText w:val="%1\."/.test(match[0]));
  const decimalAbstract = [...decimalAbstracts].reverse().find((match) => /w:hanging="270"/.test(match[0])) ?? decimalAbstracts.at(-1);
  const decimalNum = [...numberingXml.matchAll(/<w:num w:numId="(\d+)"[\s\S]*?<\/w:num>/g)]
    .find((match) => new RegExp(`<w:abstractNumId w:val="${decimalAbstract[1]}"`).test(match[0]));
  assert.match(documentXml, new RegExp(`<w:numId w:val="${decimalNum[1]}"`));
  assert.match(decimalAbstract[0], /w:hanging="270"/);
...
test("a citation uses only its own paragraph date when ranking source documents", async () => {
  const analysis = await analyseParagraphs([
    "The notice to proceed was dated 1 December 2025.",
    "The grid connection offer dated 14 October 2025 records the energisation sequence. A copy is at exhibit [LV1/xx].",
    "The termination notice was dated 30 April 2026.",
  ], [
    new File(["Notice to proceed dated 1 December 2025"], "Notice_to_Proceed_2025-12-01.txt"),
    new File(["Grid connection offer dated 14 October 2025 energisation sequence"], "Grid_Connection_Offer_2025-10-14.txt"),
    new File(["Termination notice dated 30 April 2026"], "Termination_Notice_2026-04-30.txt"),
  ]);
  const candidate = analysis.candidates[0];
  assert.equal(candidate.date, "14 October 2025");
  assert.equal(analysis.evidence.find((record) => record.id === candidate.evidenceId)?.name, "Grid_Connection_Offer_2025-10-14.txt");
});

test("a repeated citation without a date does not borrow a neighbouring document date", async () => {
  const analysis = await analyseParagraphs([
    "The termination notice was dated 30 April 2026.",
    "I refer again to the executed EPC contract because its termination provisions govern. It is exhibited again as [LV1/xx].",
    "The variation order was dated 22 January 2026.",
  ], [
    new File(["Executed EPC contract termination provisions"], "EPC_Contract_Executed.txt"),
    new File(["Termination notice dated 30 April 2026"], "Termination_Notice_2026-04-30.txt"),
    new File(["Variation order dated 22 January 2026"], "Variation_Order_2026-01-22.txt"),
  ]);
  const candidate = analysis.candidates[0];
  assert.equal(candidate.date, "Date not stated");
  assert.equal(candidate.evidenceId, null, "a repeat without enough same-paragraph evidence may stay unselected, but must not borrow a neighbour's date");
});
