...
test("legal pages restart by volume and optional pages count only when selected", async () => {
  const { createBuildPlan } = await import("../app/lib/build-plan.ts");
  ...
  assert.deepEqual(excluded.volumes.map((volume) => volume.items[0].legalStartPage), [1, 1]);
  assert.deepEqual(counted.volumes.map((volume) => volume.items[0].legalStartPage), [2, 2]);
