...
test("release promotion is gated by source, package, unpacked and installed verification", async () => {
  ...
  assert.match(gate, /legal-integrity coverage/);
  ...
  assert.match(gate, /if \(!existsSync\(privateStressPack\)\)/);
  assert.match(gate, /unpacked shell smoke test/);
  assert.match(gate, /unpacked guided-sample finished-bundle smoke test/);
  assert.match(gate, /--build-smoke-test/);
  ...
  assert.match(installed, /--build-smoke-test/);
  assert.match(installed, /finished-bundle build, reopen and save passed/);
