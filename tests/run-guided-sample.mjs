...
const { samplePackRelativePath } = await import("../app/lib/bundle-engine.ts");
...
async function fixture(name, overrideUrl) {
  const url = overrideUrl ?? new URL(samplePackRelativePath(name), sampleRoot);
...
async function colourWorkbookPreview(_file, sheets) {
  return Promise.all(sheets.map(async (sheet) => {
    ...
    for (let pageIndex = 1; pageIndex < (sheet.expectedPageCount ?? 1); pageIndex += 1) pdf.addPage([841.89, 595.28]).drawText(`SAMPLE COST WORKBOOK - ${sheet.name} - page ${pageIndex + 1}`, { x: 58, y: 474, size: 16, font: bold });
...
test("builds the complete guided sample with mixed evidence, one selected worksheet and supplied templates", async () => {
  ...
  const statementUrl = process.env.EXHIBIT_GUIDED_STAGED_STATEMENT === "1" ? stagedStatement : new URL(samplePackRelativePath(SAMPLE_STATEMENT), sampleRoot);
  ...
  const workbookSourceBefore = process.env.EXHIBIT_BUILDER_NATIVE_EXCEL === "1" ? await readFile(new URL(samplePackRelativePath("05_SAMPLE_Cost_Workbook.xlsx"), sampleRoot)) : null;
  ...
  if (process.env.EXHIBIT_BUILDER_NATIVE_EXCEL === "1") {
    assert.equal(sha(await readFile(new URL(samplePackRelativePath("05_SAMPLE_Cost_Workbook.xlsx"), sampleRoot))), sha(workbookSourceBefore), "native Excel printing leaves the source workbook byte-for-byte unchanged");
    ...
    const workbookPages = await Promise.all(Array.from(
      { length: workbookRecord.endPage - workbookRecord.startPage + 1 },
      (_, index) => rendered.getPage(workbookRecord.startPage + index),
    ));
    const workbookText = (await Promise.all(workbookPages.map(async (page) => (
    ...
    const operatorLists = await Promise.all(workbookPages.map((page) => page.getOperatorList()));
    const colourOperations = new Set([pdfjs.OPS.setFillRGBColor, pdfjs.OPS.setStrokeRGBColor]);
    const nonGreyColour = operatorLists.some((operators) => operators.fnArray.some((operation, index) => {
