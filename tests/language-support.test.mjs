...
test("documents and enforces the English-only matching and OCR boundary", async () => {
  ...
  assert.match(readme, /English DOCX statement recognition, matching and local OCR are the tested/);
  assert.match(readme, /Non-Latin and right-to-left matching are not supported/);
  assert.match(readme, /Full multilingual matching, OCR language packs, fonts and right-to-left/);
});
