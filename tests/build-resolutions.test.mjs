...
test("OCR exception does not apply to a different source with the same hash", async () => {
  const { applyBuildResolutions, shouldSkipOcr } = await import("../app/lib/build-resolutions.ts");
  const hash = "a".repeat(64);
  const resolution = {
    blockerId: "ocr-source-a",
    checkCode: "ocr.unavailable",
    action: "proceed-without-ocr",
    sourceId: "source-a",
    sourceSha256: hash,
  ...
  const effective = applyBuildResolutions(
    [
      { id: "ocr-source-a", code: "ocr.unavailable", policy: "exception-eligible", severity: "blocking", label: "OCR unavailable", detail: "Needs OCR.", sourceId: "source-a", sourceSha256: hash, fileName: "scan.pdf" },
      { id: "ocr-source-b", code: "ocr.unavailable", policy: "exception-eligible", severity: "blocking", label: "OCR unavailable", detail: "Needs OCR.", sourceId: "source-b", sourceSha256: hash, fileName: "scan.pdf" },
    ],
    [resolution],
  );
  assert.equal(effective[0].severity, "warning");
  assert.equal(effective[1].severity, "blocking");
  assert.equal(shouldSkipOcr("source-a", hash, [resolution]), true);
  assert.equal(shouldSkipOcr("source-b", hash, [resolution]), false);
});
