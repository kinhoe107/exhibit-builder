...
function samplePayload(multiVolume = false) {
  return createBuildReportPayload({
    ...
    candidates: [
      { id: "c1", mark: "AH 1", provisionalNumber: 1, description: "Supply agreement", date: "8 August 2026", paragraph: 2, citation: "I refer to the agreement.", citationResolution: "none", discoverySignals: [], evidenceId: "e1", confidence: 90, rationale: "Matched", included: true, confirmed: true, confirmationMethod: "individual", confirmedAt: "2026-08-15T11:45:00.000Z" },
...
test("technical JSON payload keeps the existing report shape", () => {
  ...
  assert.equal(payload.review[0].confidence, 90);
  assert.equal(payload.review[0].rationale, "Matched");
  assert.equal(payload.review[0].confirmationMethod, "individual");
  assert.equal(payload.review[0].confirmedAt, "2026-08-15T11:45:00.000Z");
});
...
test("readable UTF-8 report covers exhibits, hashes, manuals, exclusions, warnings and volumes", () => {
  ...
  assert.match(text, /Match review provenance/);
  assert.match(text, /Comparison score 90\/100/);
  assert.match(text, /Rationale: Matched/);
  assert.match(text, /Confirmed individually at 2026-08-15T11:45:00.000Z/);
