...
try {
  ...
  if (!/EXHIBIT BUILDER FREE INDIVIDUAL END USER LICENCE AGREEMENT/i.test(eulaText)) fail("the installed EULA is not the approved Exhibit Builder licence");
  ...
  const smokeSave = path.join(temporaryRoot, `ExhibitBuilderReleaseVerify-save-${version}-${process.pid}`);
  mkdirSync(smokeSave, { recursive: true });
  run(executable, ["--build-smoke-test"], 15 * 60 * 1000, { ...smokeEnvironment, EXHIBIT_BUILDER_SMOKE_SAVE_DIR: smokeSave });
  const savedPdfs = existsSync(smokeSave) ? readdirSync(smokeSave).filter((name) => /\.pdf$/i.test(name)) : [];
  if (!savedPdfs.length) fail("the installed build smoke test did not save a finished PDF");
  const savedPdf = readFileSync(path.join(smokeSave, savedPdfs[0]));
  if (!savedPdf.subarray(0, 5).equals(Buffer.from("%PDF-"))) fail("the installed finished PDF is not a PDF");
  if (savedPdf.byteLength < 10_000) fail("the installed finished PDF is too small to be a complete bundle");
  checks.push("installed finished-bundle build, reopen and save passed");
