...
} else if (process.env.EXHIBIT_BUILDER_REQUIRE_PRIVATE_STRESS === "1") {
  throw new Error("The required private ICC adversarial pack is missing. Release promotion is blocked.");
} else {
  process.stdout.write("\n[release gate] private ICC adversarial pack not present; public-source verification continues without private fixtures\n");
}
...
run("unpacked guided-sample analysis smoke test", unpackedExecutable, ["--analysis-smoke-test"], 10 * 60 * 1000, smokeEnvironment);
run("temporary installation, installed smoke tests and uninstall", node, [path.join("scripts", "verify-installed-release.cjs"), installer, version, path.join(root, "release", `release-verification-${version}.json`)], 20 * 60 * 1000);
