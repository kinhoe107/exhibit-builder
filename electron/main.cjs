...
const { appendFileSync, mkdirSync, mkdtempSync, readdirSync, rmSync, statSync } = require("node:fs");
...
installDiagnosticStreamErrorHandlers([process.stdout, process.stderr], {
  ...
  fatal(error) {
    process.exitCode = 1;
    if (!SMOKE_TEST && !ANALYSIS_SMOKE_TEST) {
      dialog.showErrorBox(
...
diagnostic("module-loaded", { smoke: SMOKE_TEST, analysisSmoke: ANALYSIS_SMOKE_TEST });
...
function showUnresponsiveDialog(window) {
  if (unresponsiveDialogOpen || window.isDestroyed() || SMOKE_TEST || ANALYSIS_SMOKE_TEST) return;
...
function attachWindowDiagnostics(window) {
  ...
  window.webContents.on("render-process-gone", (_event, details) => {
    ...
    if (SMOKE_TEST || ANALYSIS_SMOKE_TEST || window.isDestroyed()) return;
...
function makeWindow() {
  ...
  if (ANALYSIS_SMOKE_TEST) window.webContents.on("console-message", (_event, level, message, line, sourceId) => console.error(`[analysis-smoke renderer ${level}] ${message} (${sourceId}:${line})`));
  if (ANALYSIS_SMOKE_TEST) window.webContents.on("did-fail-load", (_event, code, description, url) => console.error(`[analysis-smoke load failure] ${code} ${description} ${url}`));
...
async function createWindow() {
  ...
  mainWindow = window;
  diagnostic("window-ready", { smoke: SMOKE_TEST, analysisSmoke: ANALYSIS_SMOKE_TEST });
  if (SMOKE_TEST || ANALYSIS_SMOKE_TEST) {
    diagnostic("smoke-check-start");
    const result = ANALYSIS_SMOKE_TEST
          ...
          (async () => {
            ...
            while (Date.now() < deadline) {
              ...
              if (error || (candidateRows === 5 && citedReferenceCount === 6)) {
                ...
                await wait(0);
                const useNumbering = Array.from(document.querySelectorAll("button")).find(
                ...
                while (secondPicker && !secondPicker.querySelector('[role="option"]') && Date.now() < pickerDeadline) {
                  await wait(50);
                }
                const pickerOptionCount = secondPicker?.querySelectorAll('[role="option"]').length ?? 0;
                const replacement = Array.from(secondPicker?.querySelectorAll('[role="option"]') ?? []).find(
                ...
                const emailReview = Array.from(emailCard?.querySelectorAll("button") ?? []).find((button) => button.textContent.trim() === "Review");
                emailReview?.click();
                await wait(0);
                ...
                Array.from(emailCard?.querySelectorAll("button") ?? []).find((button) => button.textContent.trim() === "Review")?.click();
                await wait(0);
                ...
                return {
                  ...
                  analysisPassed: candidateRows === 5 && citedReferenceCount === 6 && guidedIdentity && numberingInputPassed && standardTemplateStatePassed && templateSelectionPassed && templateResetPassed && documentPickerPassed && repeatReviewPassed && originalPdfPreviewPassed && templatePreviewPassed && confirmScrollPassed && confirmFocusPassed && attachmentRowCount > 0 && emailChoicesMade === attachmentRowCount && emailCompact && emailAttachmentsOpenBeforeMinimise && emailMinimiseAfterReview && emailDecisionsBeforeMinimise && emailCompactAfterMinimise && emailAttachmentsClosedAfterMinimise && emailDecisionsAfterMinimise && emailDecisionsAfterUndo && emailCompactAfterAttachmentsFirstConfirm && repeatProgressPassed && !error,
                  candidateRows,
                  repeatRows,
                  referenceRows,
                  citedReferenceCount,
              ...
              if (
                ...
                currentSampleButton.textContent.includes("Try the guided sample") &&
                Date.now() - lastStartAttempt >= 1000
              ) {
                currentSampleButton.click();
    ...
    const passed =
      ...
      (!ANALYSIS_SMOKE_TEST || result.analysisPassed);
    ...
    diagnostic("smoke-check-finished", { passed, hasWorkspace: result.hasWorkspace, analysisPassed: result.analysisPassed });
...
ipcMain.handle(
  ...
  async (event, { bytes, fileName, mediaType }) => {
