...
function matchStrengthLabel(score: number, rationale?: string) {
  if (rationale === "Source selected by reviewer") return "Selected by you";
  if (score >= 90) return "Strong suggested match";
  if (score >= 65) return "Possible suggested match";
  return "Needs review";
}
...
function numberingDiffersFromPdfOrder(settings: PageNumberSettings) {
  return !settings.matchPdfPageOrder || Boolean(settings.prefix || settings.suffix || settings.padding > 0);
}

function numberingDifferenceExample(settings: PageNumberSettings) {
  if (settings.matchPdfPageOrder) {
    const numeric = settings.padding ? String(1).padStart(settings.padding, "0") : "1";
    return `PDF page 1 will show ${settings.prefix}${numeric}${settings.suffix}. The number itself continues in the same order as the PDF viewer.`;
  }
  if (settings.scheme === "section") return "An exhibit page may show AH1-1 while the PDF viewer shows its physical page position.";
  if (settings.scheme === "bates" || settings.prefix || settings.suffix || settings.padding > 0 || settings.startAt !== 1) {
    const numeric = settings.padding ? String(settings.startAt).padStart(settings.padding, "0") : String(settings.startAt);
    return `The first exhibit page will show ${settings.prefix}${numeric}${settings.suffix}, regardless of its physical PDF page position.`;
  }
  if (!settings.countTemplates || settings.preliminary === "none") return "Cover or index pages may have no printed number.";
  if (settings.preliminary === "roman") return "Cover and index pages use Roman numerals; exhibit numbering restarts separately.";
  return "Exhibit numbering restarts separately after the cover and index.";
}
...
export default function BundleBuilder() {
  ...
  const [reorderReturn, setReorderReturn] = useState<{ build: BuildResult; arrangement: BundleArrangement; candidates: ExhibitCandidate[]; pageSizeChoices: Record<string, NonA4PageHandling> } | null>(null);
  ...
  const repeatCitationCandidateIds = useMemo(
    () => new Set(exhibitGroups.flatMap((group) => group.collisionMembers.slice(1).map((candidate) => candidate.id))),
    [exhibitGroups],
  );
  const bulkConfirmableCandidates = useMemo(
    () => candidates.filter((candidate) => {
      if (candidate.parentEmailProvenance) return false;
      const group = exhibitGroups.find((item) => item.canonical.id === candidate.id);
      return !repeatCitationCandidateIds.has(candidate.id) && candidate.included && Boolean(candidate.evidenceId) && !candidate.confirmed && (!group || (!group.decisionPending && !group.selectionConflict));
    }),
    [candidates, exhibitGroups, repeatCitationCandidateIds],
  );
  ...
  const nonA4Exhibits = useMemo(() => {
    ...
    return exhibitGroups.flatMap((group) => {
      ...
      for (const child of emailChildrenForDisposition(record.emailAttachments, group.canonical.emailAttachmentDispositions, "print-with-email")) {
        ...
        const nonA4 = child.pageSizes.filter((page) => !page.isA4);
  ...
  const confirmedWorkbookExhibits = useMemo(() => {
    ...
    const top = analysis.evidence
      ...
      .map((record) => ({ key: record.id, name: record.name, evidenceId: record.id, workbook: record.workbook, sheetSelections: record.sheetSelections ?? [] }));
  ...
  const statementUpdateText = useMemo(
    () => build ? buildStatementUpdateSuggestions(build.volumes ? build.volumes.flatMap((volume) => volume.records) : build.records).map((item) => item.line).join("\n") : "",
    [build],
  );
  useEffect(() => {
    setCopyStatus(null);
  ...
  useEffect(() => {
    const validIds = rawExhibitGroups.map((group) => group.id);
    const insertAfter = emailChildInsertAfter(rawExhibitGroups);
    setArrangement((current) => {
      const reconciled = reconcileBundleArrangement(current, validIds, insertAfter);
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        for (const statement of statements) {
          ...
          if (sourcePath) direct.push({ id: statement.id, role: "statement", name: statement.file.name, path: sourcePath, sha256: await fileSha256(statement.file), size: statement.file.size });
        }
        ...
        const byId = new Map(recoverySourceDescriptors.current.map((source) => [source.id, source]));
  ...
  async function runAnalysis(
    ...
    try {
      ...
      const result = await analyseBundleStatements(activeStatements, evidence, reportAnalysisProgress);
      setAnalysis(result);
  ...
  async function useSamplePack() {
    ...
    try {
      ...
      const result = await analyseBundleStatements(guidedStatements, sample.evidence, reportAnalysisProgress);
      const guidedEvidence = result.evidence.map((record) => record.name === "05_SAMPLE_Cost_Workbook.xlsx"
        ? {
            ...record,
      ...
      const guidedCandidates = result.candidates.map((candidate, index) => {
        const evidence = guidedEvidence.find((record) => record.name === guidedFiles[index]);
        return {
          ...candidate,
          evidenceId: evidence?.id ?? null,
          confidence: evidence ? 100 : candidate.confidence,
          rationale: evidence ? "Guided sample mapping: this obvious source is supplied so the user can practise the approval step." : candidate.rationale,
          repeatDecision: index === 5 ? "same" as const : candidate.repeatDecision,
        };
      });
      setAnalysis({ ...result, evidence: guidedEvidence, candidates: guidedCandidates });
      setCandidates(guidedCandidates);
      setTemplates([]);
  ...
  function removeManualCandidate(candidateId: string) {
    ...
    if (group) setArrangement((current) => reconcileBundleArrangement(current, rawExhibitGroups.filter((item) => item.id !== group.id).map((item) => item.id)));
    setOrderPreview(null);
    setBuild(null);
  ...
  function excludeSource(check: PreflightCheck) {
    ...
    if (affectedCount > 1 && !window.confirm(`Exclude this source and all ${affectedCount} citations using it from the bundle?`)) return;
  ...
  function undoOrderChange() {
    ...
    setArrangement(reconcileBundleArrangement(previous, rawExhibitGroups.map((group) => group.id)));
    setOrderPreview(null);
    setBuild(null);
  ...
  async function generateBundle() {
    ...
    try {
      ...
      result = await finalizeBuildAudit(result, currentSnapshot.fingerprint, comparison);
      setBuild(result);
  ...
  function commitPagination(change: Partial<PageNumberSettings>, warn = true) {
    ...
    if (warn && numberingDiffersFromPdfOrder(next.pagination)) {
      setPaginationConfirmation(next.pagination);
      setPaginationPendingChange(change);
      setPaginationDraft(next.draft);
      return;
    }
    setPagination(next.pagination);
    setPaginationDraft(paginationDraftAfterWarningAccept(next.pagination, next.draft, change));
  }

  function acceptPaginationChange() {
    if (!paginationConfirmation) return;
    const change = paginationPendingChange ?? {};
    setPagination(paginationConfirmation);
    setPaginationDraft((current) => paginationDraftAfterWarningAccept(paginationConfirmation, current, change));
  ...
  function confirmOrderChange() {
    if (!build) return;
    setReorderReturn({ build, arrangement: cloneBundleArrangement(arrangement), candidates: candidates.map((candidate) => ({ ...candidate })), pageSizeChoices: { ...pageSizeChoices } });
    setBuild(null);
    setOrderHistory([]);
    setOrderPreview(null);
  ...
  function keepCurrentBundle() {
    if (!reorderReturn) return;
    setArrangement(reorderReturn.arrangement);
    setCandidates(reorderReturn.candidates);
    setPageSizeChoices(reorderReturn.pageSizeChoices);
    setBuild(reorderReturn.build);
    setReorderReturn(null);
    setOrderHistory([]);
    setOrderPreview(null);
  }
  ...
  async function chooseTemplate(slot: TemplateSlot, event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!["pdf", "docx", "doc"].includes(extension)) {
      input.value = "";
      setError("Templates must be PDF or Word files (.pdf, .docx or .doc).");
      return;
    }
    setBusy("template");
    setError(null);
  ...
  async function saveProject() {
    if (!analysis) return;
    try {
      const statementSources = await Promise.all(statements.map(async (statement) => {
        const digest = await crypto.subtle.digest("SHA-256", await statement.file.arrayBuffer());
        return { id: statement.id, role: "statement" as const, name: statement.file.name, sha256: Array.from(new Uint8Array(digest)).map((part) => part.toString(16).padStart(2, "0")).join(""), file: statement.file };
      }));
  ...
  async function applyOpenedProject(opened: Awaited<ReturnType<typeof openProjectArchive>>, _legacyTemplateApprovals?: Map<string, boolean>) {
      ...
      setPagination({ ...DEFAULT_PAGINATION, ...(opened.snapshot.pagination ?? {}) });
      setPaginationDraft({ ...DEFAULT_PAGINATION, ...(opened.snapshot.pagination ?? {}) });
      ...
      if (projectSource) {
        ...
        const restoreRecoveryDecision = (saved: (typeof decisions)[number]) => {
          if (saved.parentEmailProvenance) {
            const { id: _id, sourceSha256: _sourceSha256, ...decision } = saved;
            return { ...decision, id: saved.id } as ExhibitCandidate;
          }
          const evidence = saved.sourceSha256 ? result.evidence.find((record) => record.sha256 === saved.sourceSha256) : undefined;
          const { id: _id, sourceSha256: _sourceSha256, ...decision } = saved;
          return { ...decision, id: saved.id, evidenceId: evidence?.id ?? null, confirmed: Boolean(evidence && decision.confirmed) } as ExhibitCandidate;
        };
        const restoredCandidates = result.candidates.map((candidate) => {
          ...
          return restoreCitedCandidateDecision(candidate, saved as ExhibitCandidate, restored.evidenceId);
        });
        ...
        setPagination({ ...DEFAULT_PAGINATION, ...(payload.pagination ?? {}) });
        setPaginationDraft({ ...DEFAULT_PAGINATION, ...(payload.pagination ?? {}) });
  ...
  function templateControl(slot: TemplateSlot, label: string, emptyLabel: string) {
    ...
    return <div className="template-control">
      ...
      <input key={`${slot}-${template?.sha256 ?? "standard"}`} id={inputId} hidden={Boolean(template)} type="file" accept=".pdf,.docx,.doc" onChange={(event) => void chooseTemplate(slot, event)} />
  ...
  function renderManualExhibitPanel() {
    ...
    return <section className="manual-exhibit-panel" role="dialog" aria-labelledby="manual-exhibit-title">
      <div><h3 id="manual-exhibit-title">Add an exhibit</h3><p>Choose an unused supplied document, or another local file.</p></div>
      ...
      <p className="manual-exhibit-note">This exhibit is not cited in the statement. No statement reference is invented. It is listed at the end of the copy-ready suggestions as an uncited exhibit.</p>
  ...
  function renderArrangementExhibit(exhibitId: string, sectionId: string | null, index: number, containerLength: number) {
    ...
    return <li key={exhibitId} data-exhibit-id={exhibitId} className={`finalise-order-item ${draggingExhibitId === exhibitId ? "dragging" : ""}`} draggable={!orderPreview && !draggingSectionId && !glued} onDragStart={() => { if (glued || draggingSectionId) return; setDraggingExhibitId(exhibitId); }} onDragEnd={() => setDraggingExhibitId(null)} onDragOver={(event) => { if (!orderPreview && !draggingSectionId && !glued) event.preventDefault(); }} onDrop={() => { if (glued || draggingSectionId) return; if (draggingExhibitId && !orderPreview) moveCandidateBefore(draggingExhibitId, exhibitId); setDraggingExhibitId(null); }}>
      ...
      <div className="finalise-order-actions">
        {glued ? null : <button data-action="top" type="button" disabled={Boolean(orderPreview) || index === 0} onClick={() => moveCandidateToEdge(exhibitId, "top")}>Top</button>}
        <button data-action="earlier" type="button" disabled={Boolean(orderPreview) || index === 0} onClick={() => moveCandidate(exhibitId, -1)}>Earlier</button>
        <button data-action="later" type="button" disabled={Boolean(orderPreview) || index === containerLength - 1} onClick={() => moveCandidate(exhibitId, 1)}>Later</button>
        {glued ? null : <button data-action="bottom" type="button" disabled={Boolean(orderPreview) || index === containerLength - 1} onClick={() => moveCandidateToEdge(exhibitId, "bottom")}>Bottom</button>}
  ...
  function navigateToBlocker(blocker: BuildBlocker) {
    ...
    if (blocker.candidateId) {
      ...
      setExpandedConfirmedCards((current) => new Set(current).add(blocker.candidateId!));
      if (emailAttachment) setOpenEmailAttachmentsId(blocker.candidateId);
      focusBlockerTarget(emailAttachment
        ? emailAttachmentsSelector(blocker.candidateId)
        : reviewCardSelector(blocker.candidateId));
      return;
    }
  ...
  return (
    <main className="app-shell">
      ...
      <header className="topbar">
        ...
        {analysis && (
          <>
            <button className="text-button" type="button" onClick={saveProject}>Save exhibit project</button>
          <button className="text-button" type="button" onClick={() => { if (!analysis || window.confirm("Start a new exhibit project? This will discard the current statement, exhibit matches, approvals and build settings.")) resetWorkspace(); }}>New exhibit project</button>
          </>
        )}
      </header>
      ...
      {!analysis ? (
        <section className="welcome">
          <div className="welcome-copy">
            ...
            {guidedSamplePreferenceReady && !guidedSampleHidden ? <div className="guided-sample-panel">
              {guidedSampleAvailability !== "unavailable" ? <div className="hero-actions">
                <button
                  className="primary-button"
                  type="button"
                  onClick={useSamplePack}
                  disabled={busy !== null || guidedSampleAvailability !== "available"}
                  data-testid="guided-sample-button"
                >
                  {busy === "sample" || busy === "analyse"
                    ? "Analysing sample"
                ...
                <span className="hero-note">
                  Guided sample with [Exhibit] placeholders, several exhibits in one paragraph, Excel, repeats and an uncited practice file.
                </span>
              </div> : <div className="guided-sample-unavailable" role="status"><div><strong>The optional guided sample is unavailable.</strong><span>You can still build a bundle from your own files.</span></div><button className="secondary-button compact-action" type="button" onClick={() => void retryGuidedSampleAvailability()}>Check again</button></div>}
              <button className="text-button guided-sample-hide" type="button" onClick={() => void setGuidedSampleHidden(true)}>Hide the guided sample from this screen</button>
            </div> : null}
          </div>
          ...
          <div className="upload-panel">
            ...
            <div className="upload-footer">
              ...
              {analysisProgress ? <div className="analysis-progress"><strong>{analysisProgress.stage}</strong><span>{analysisProgress.detail}</span></div> : null}
              <button
                className="primary-button"
                type="button"
        ...
        <section className="workspace">
          <aside className="workspace-nav">
            ...
            <nav aria-label="Bundle workflow">
              ...
              <button
                ...
                type="button"
                onClick={() => setView(readyToLeaveReview && includedWorkbookCount ? "sheets" : "review")}
                disabled={!readyToLeaveReview || !includedWorkbookCount}
                aria-current={view === "sheets" ? "step" : undefined}
                title={!readyToLeaveReview ? "Complete exhibit review before selecting workbook sheets." : !includedWorkbookCount ? "No confirmed workbook exhibit is ready for sheet selection." : undefined}
              >
                <span>03</span>
                <span>
                  <strong>Sheets</strong><small>{workbookSourceCount ? `${includedWorkbookCount} of ${workbookSourceCount} confirmed` : "No workbook files"}</small>
                </span>
              </button>
          ...
          <div className="workspace-main">
            {view === "sources" && (
              <>
                <div className="workspace-header"><div><p className="eyebrow">Stage 1 of 4</p><h1>Sources</h1><p>Statements are read-only. Evidence stays local and nothing is included until reviewed.</p></div><button className="primary-button" onClick={() => setView("review")}>Continue to exhibit review</button></div>
            ...
            {view === "sheets" && (
              <>
                <div className="workspace-header"><div><p className="eyebrow">Stage 3 of 4</p><h1>Choose which Excel sheets to include</h1><p>An Excel workbook can contain several worksheet tabs. Tick only the tabs that belong in this exhibit. Unticked tabs stay in the source workbook but are omitted from the bundle.</p></div><div className="workspace-header-actions"><button className="secondary-button" type="button" onClick={() => setView("review")}>Back to exhibit review</button><button className="primary-button" type="button" onClick={() => setView("build")} disabled={!readyToBuild || busy !== null}>Continue to finalise</button></div></div>
                {!readyToBuild && busy === null && <section className="build-readiness sheet-readiness" aria-live="polite" data-testid="sheet-readiness">
            ...
            {view === "review" && (
              <>
                ...
                <details id="optional-settings" tabIndex={-1} className="statement-safety-note" aria-label="Optional project and bundle settings">
                  ...
                  <div className="advanced-settings">
                    ...
                    <section className="advanced-settings-section">
                      <h3>Project and page numbering</h3>
                      ...
                      <div className="advanced-settings-grid"><label className="wide">Exhibit project name<input value={projectName} onChange={(event) => setProjectName(event.target.value)} /><small>Used for the saved project, crash recovery and report filenames. It is not printed on the bundle cover. Most projects can leave this unchanged.</small></label></div>
                      <fieldset className="numbering-mode"><legend>How should printed page numbers work?</legend>
                        <label><input type="radio" name="numbering-mode" checked={paginationDraft.matchPdfPageOrder} onChange={() => commitPagination({ matchPdfPageOrder: true, countTemplates: true, preliminary: "arabic", scheme: "bundle", startAt: 1 })} /><span><strong>One sequence across the whole bundle — recommended</strong><small>{suppliedCoverOmitsPageNumber ? "This cover stays unnumbered unless you tick “Print a page number on this cover”. The index continues the sequence, and exhibit pages follow without restarting. If the bundle is split, the sequence continues into the next volume." : "The cover is page 1, the index continues the sequence, and exhibit pages follow without restarting. If the bundle is split, the sequence continues into the next volume."}</small></span></label>
                        <label><input type="radio" name="numbering-mode" checked={!paginationDraft.matchPdfPageOrder} onChange={() => commitPagination({ matchPdfPageOrder: false })} /><span><strong>Separate numbering for exhibit pages</strong><small>Choose this only when cover and index pages must use a different sequence, or remain unnumbered.</small></span></label>
                      </fieldset>
                      {paginationDraft.matchPdfPageOrder ? <><div className="advanced-settings-grid continuous-numbering-controls">
                      ...
                      </div><div className="numbering-preview" aria-live="polite"><strong>Example across the finished bundle</strong><span>{`${paginationDraft.prefix}${paginationDraft.padding ? String(1).padStart(paginationDraft.padding, "0") : "1"}${paginationDraft.suffix}, ${paginationDraft.prefix}${paginationDraft.padding ? String(2).padStart(paginationDraft.padding, "0") : "2"}${paginationDraft.suffix}, ${paginationDraft.prefix}${paginationDraft.padding ? String(3).padStart(paginationDraft.padding, "0") : "3"}${paginationDraft.suffix}…`}</span><small>{suppliedCoverOmitsPageNumber ? "Index and exhibit pages use this continuous sequence. This cover stays unnumbered unless you tick “Print a page number on this cover”." : "Cover, index and exhibit pages use this one continuous sequence, including across separate volumes."}</small></div><div className="numbering-apply-row"><button data-testid="apply-page-numbering" className="secondary-button" type="button" disabled={!paginationDraftChanged} onClick={() => commitPagination({})}>Apply page-number changes</button><small>{paginationDraftChanged ? "Review the example, then apply these changes. A warning appears before numbering that differs from the PDF viewer is used." : "The displayed page-number settings are applied."}</small></div></> : <><div className="advanced-settings-grid">
                        <label>Exhibit-page number format<select value={paginationDraft.scheme} onChange={(event) => commitPagination({ scheme: event.target.value as PageNumberSettings["scheme"] })}><option value="bundle">Continuous numbers (1, 2, 3)</option><option value="bates">Custom prefix and number (ABC-0001)</option><option value="section">Exhibit mark and page (AH1-1)</option></select><small>{paginationDraft.scheme === "bundle" ? "One continuous printed sequence runs through the exhibit pages." : paginationDraft.scheme === "bates" ? "The prefix, starting number and minimum digits create the printed number." : "The exhibit mark is added automatically. Prefix and suffix do not apply to this format."}</small></label>
                        {paginationDraft.scheme !== "section" ? <label>Printed-number prefix<input data-testid="page-number-prefix" value={paginationDraft.prefix} onChange={(event) => setPaginationDraft((current) => updatePaginationDraft(current, { prefix: event.target.value }))} placeholder="For example, ABC-" /><small>Placed directly before the number.</small></label> : null}
                        <label>First exhibit-page number<input data-testid="page-number-start" type="number" min="1" value={paginationDraft.startAt} onChange={(event) => setPaginationDraft((current) => updatePaginationDraft(current, { startAt: Math.max(1, Number(event.target.value) || 1) }))} /></label>
                        {paginationDraft.scheme !== "section" ? <label>Printed-number suffix<input data-testid="page-number-suffix" value={paginationDraft.suffix} onChange={(event) => setPaginationDraft((current) => updatePaginationDraft(current, { suffix: event.target.value }))} placeholder="Optional" /><small>Placed directly after the number.</small></label> : null}
                        <label>Minimum number of digits<input data-testid="page-number-padding" type="number" min="0" max="12" value={paginationDraft.padding} onChange={(event) => setPaginationDraft((current) => updatePaginationDraft(current, { padding: Math.max(0, Math.min(12, Number(event.target.value) || 0)) }))} /><small>0 gives 1, 2, 3. Four digits gives 0001, 0002, 0003.</small></label>
                        {paginationDraft.scheme !== "section" && (paginationDraft.prefix || paginationDraft.suffix) ? <p className="settings-note">The index and suggested statement references use the same complete page labels, including this prefix and suffix.</p> : null}
                        <label className="checkbox-field wide"><input type="checkbox" checked={paginationDraft.countTemplates} onChange={(event) => commitPagination({ countTemplates: event.target.checked, preliminary: event.target.checked && pagination.preliminary === "none" ? "roman" : pagination.preliminary })} /> Print numbers on cover and index pages <small>Turn this off to leave those preliminary pages unnumbered.</small></label>
                        {paginationDraft.countTemplates ? <label>Cover and index number style<select value={paginationDraft.preliminary === "none" ? "roman" : paginationDraft.preliminary} onChange={(event) => commitPagination({ preliminary: event.target.value as PageNumberSettings["preliminary"] })}><option value="roman">Roman numerals (i, ii, iii)</option><option value="arabic">Standard numbers (1, 2, 3)</option></select><small>This preliminary sequence is separate from the exhibit-page sequence.</small></label> : null}
                      </div><div className="numbering-preview" aria-live="polite"><strong>Example of the selected legal numbering</strong><span>{paginationDraft.scheme === "section" ? "AH1-1, AH1-2, AH2-3" : `${paginationDraft.prefix}${paginationDraft.padding ? String(paginationDraft.startAt).padStart(paginationDraft.padding, "0") : paginationDraft.startAt}${paginationDraft.suffix}`}</span></div><div className="numbering-apply-row"><button data-testid="apply-page-numbering" className="secondary-button" type="button" disabled={!paginationDraftChanged} onClick={() => commitPagination({})}>Apply page-number changes</button><small>{paginationDraftChanged ? "Review the example, then apply these changes. A warning appears before numbering that differs from the PDF viewer is used." : "The displayed page-number settings are applied."}</small></div></>}
                    </section>
                    <section className="advanced-settings-section">
                      <h3>Page numbers shown in the PDF</h3>
                      ...
                      {numberingDiffersFromPdfOrder(pagination) ? <div className="numbering-divergence" role="status"><strong>Printed numbering differs from PDF page order.</strong><span>{numberingDifferenceExample(pagination)}</span><small>The index and suggested statement references will use the printed legal numbering.</small></div> : null}
                    </section>
                    <section className="advanced-settings-section template-settings-section">
                      ...
                      <div className="template-subsection">
                        ...
                        <label className="reference-counting-option"><input type="checkbox" disabled={!layout.includeDividerPages && !layout.includeExhibitCoverPages} checked={layout.countOptionalPagesInReferences} onChange={(event) => setLayout((current) => ({ ...current, countOptionalPagesInReferences: event.target.checked }))} /><span><strong>Include optional pages when calculating exhibit page references</strong><small>Usually left off: [AH1/1] then points to the first page of the source exhibit, not its divider or exhibit-cover page.</small></span></label>
                      </div>
                ...
                <div className="manifest-card">
                  <div className="review-list-toolbar">
                    ...
                    <div className="review-list-controls">
                      ...
                      {bulkConfirmableCount > 0 ? <button className="secondary-button compact-action bulk-confirm-action" type="button" onClick={confirmMatchedExhibits}>Confirm all {bulkConfirmableCount} proposed match{bulkConfirmableCount === 1 ? "" : "es"}</button> : null}
                    </div>
                  </div>
                          ...
                          <article className={`exhibit-review-card ${!candidate.included ? "excluded" : ""} ${compact ? "compact" : ""}`} key={candidate.id} data-candidate-id={candidate.id} data-included={candidate.included ? "true" : "false"} data-confirmed={candidate.confirmed ? "true" : "false"} data-confirmable={candidate.included && candidate.evidenceId ? "true" : "false"} tabIndex={-1}>
                            ...
                            <div className="exhibit-card-grid">
                              ...
                              <section className="primary-review-panel">
                                ...
                                <details className="document-picker" open={openDocumentPickerId === candidate.id} onToggle={(event) => { if (event.currentTarget.open) setOpenDocumentPickerId(candidate.id); else setOpenDocumentPickerId((current) => current === candidate.id ? null : current); }}>
                                  ...
                                  {openDocumentPickerId === candidate.id ? <div className="document-picker-options" role="listbox" aria-label={`Available documents for review list #${itemNumber}`}>
                                    <button type="button" role="option" aria-selected={!candidate.evidenceId} className={!candidate.evidenceId ? "selected" : ""} onClick={(event) => { if (candidate.evidenceId) updateCandidate(candidate.id, { evidenceId: null, confidence: 0, rationale: "No source file matched", repeatDecision: "pending", confirmed: false, confirmationMethod: undefined, confirmedAt: undefined }); restoreDocumentPickerSummary(event.currentTarget); setOpenDocumentPickerId(null); }}>No document selected</button>
                                    {analysis.evidence.filter((item) => !item.derivedFromEmail).map((item) => {
                                      ...
                                      return <button type="button" role="option" aria-selected={selected} className={selected ? "selected" : ""} key={item.id} onClick={(event) => { if (!selected) updateCandidate(candidate.id, { evidenceId: item.id, confidence: 100, rationale: "Source selected by reviewer", repeatDecision: "pending", confirmed: false, confirmationMethod: undefined, confirmedAt: undefined }); restoreDocumentPickerSummary(event.currentTarget); setOpenDocumentPickerId(null); }}>{item.name}{duplicateLabel}</button>;
                                    })}
                                  </div> : null}
                                </details>
                                ...
                                {record.emailAttachments.map((child) => {
                                  ...
                                  return <li key={child.identity}>
                                    ...
                                    <div className="email-attachment-actions" role="group" aria-label={`Treatment of ${child.name}`}>
                                      {child.supported ? <>
                                        <button type="button" aria-pressed={disposition === "print-with-email"} className={disposition === "print-with-email" ? "selected" : ""} onClick={() => setEmailChildDisposition(candidate.id, child, "print-with-email")}>Print with this email</button>
                            ...
                            <details className="supporting-review-details">
                              ...
                              <div className="supporting-review-grid">
                                <section><span className="field-label">Why this document was selected</span><p className="supporting-copy">{candidate.rationale}</p><p className="supporting-copy">{candidate.rationale === "Source selected by reviewer" ? "Selected by you; confirmation is still required." : <>Automated comparison score: {candidate.confidence}/100. This is a ranking aid, not the probability that the document is correct.</>}</p>{candidate.discoverySignals.length ? <p className="supporting-copy">Statement signals: {candidate.discoverySignals.join(", ")}.</p> : null}{candidate.alternativeEvidenceIds?.length ? <p className="supporting-copy">Alternatives: {candidate.alternativeEvidenceIds.map((id) => evidenceById.get(id)?.name).filter(Boolean).join("; ")}.</p> : null}</section>
                ...
                {technicalBuildBlockers.length > 0 && busy === null && <details className="build-readiness" data-testid="build-readiness" id="build-readiness" defaultOpen={technicalBuildBlockers.some((blocker) => !blocker.candidateId)}>
                  ...
                  <div className="readiness-list readiness-jump-list">{technicalBuildBlockers.map((blocker) => {
                    ...
                    return <div key={blocker.id} data-blocker-id={blocker.id} tabIndex={-1} className={`readiness-item ${blocker.kind}`}>
                      <span aria-hidden="true">!</span>
                      <div className="readiness-copy"><p className="readiness-label"><strong>{blocker.label}</strong></p><p className="readiness-detail">{blocker.fileName ? `${blocker.fileName}: ` : ""}{blocker.detail}</p>
                        <div className="readiness-controls">
                          ...
                          {blocker.candidateId ? <button className="secondary-button compact-action" type="button" onClick={() => navigateToBlocker(blocker)}>{(() => {
                          ...
                          {check && check.sourceId && !emailAttachment ? <button className="secondary-button compact-action" type="button" onClick={() => excludeSource(check)}>Exclude source and its citations</button> : null}
                        </div>
                      </div>
                    </div>;
                  })}</div>
                ...
                <div className="review-sticky-bar" aria-label="Review navigation">
                  ...
                  <button className="primary-button" type="button" onClick={() => setView(includedWorkbookInMatter ? "sheets" : "build")} disabled={!readyToLeaveReview || busy !== null} data-testid="build-bundle" aria-describedby="review-continue-status" title={reviewContinueReason ?? undefined}>{includedWorkbookInMatter ? "Continue to workbook sheets" : "Continue to finalise"}</button>
                </div>
              </>
            )}
              ...
              <>
                <div className="workspace-header">
                  <div>
                    ...
                    <h1>Reconcile the evidence inbox</h1>
                    <p>
              ...
              <>
                <div className="workspace-header">
                  <div>
                    ...
                    <h1>{build ? "Your bundle is ready" : "Finalise the exhibit bundle"}</h1>
                    <p>
                      {build
                  ...
                  {!build && (
                    <div className="workspace-header-actions"><button
                      ...
                      disabled={!readyToBuild || busy !== null}
                    ...
                    </button>{reorderReturn ? <button className="secondary-button" type="button" disabled={busy !== null} onClick={keepCurrentBundle}>Discard arrangement changes and keep current bundle</button> : null}
                ...
                {reorderReturn ? <div className="regeneration-warning" role="status"><strong>The current bundle is retained while you edit a new arrangement.</strong><p>Regenerating will replace the current result in this project and update the PDF, index, bookmarks, links, visible page numbers and witness-statement reference suggestions. Files you already downloaded are unaffected. Uploaded sources and review decisions are retained.</p></div> : null}

                {!build && !readyToBuild && <section className="build-readiness finalise-readiness" aria-live="polite" data-testid="finalise-readiness" id="finalise-build-help">
                  ...
                  <div className="check-list">
                    {preflight.map((check) => (
                      <div key={check.id}>
                        ...
                        <p><strong>{check.label}</strong><small>{check.fileName ? `${check.fileName}: ` : ""}{check.detail}</small></p>
                      </div>
                    ))}
                  </div>
                </details>}
                  ...
                  <div className="build-layout final-output-layout">
                    <section className="bundle-ready-card">
                      ...
                      <div className="bundle-summary">
                        ...
                        <div className="output-metrics" aria-label="Output summary">
                          ...
                          <span><b>{layout.countOptionalPagesInReferences ? "Yes" : "No"}</b> optional pages included in exhibit references</span>
                    ...
                    <section className="statement-update-card">
                      <div>
                        ...
                        <h2>Copy-ready references for the witness statement</h2>
                        <p>These are suggestions only. The original witness statement remains unchanged.</p>
                      </div>
                      <pre>{statementUpdateText || "No final exhibit page ranges are available."}</pre>
                      <div className="download-row">
                        <button className="secondary-button" type="button" onClick={() => void (async () => {
                          ...
                          setCopyStatus(result.copied
                            ? { kind: "success", message: "Copied to the clipboard." }
                    ...
                    <details className="completed-checks">
                      ...
                      <div className="completed-check-grid">
                        <section>
                          <h3>Preflight</h3>
                          <div className="check-list">
                            {preflight.map((check) => <div key={check.id}><span className={check.severity === "pass" ? "check-pass" : "check-warning"} aria-hidden="true">{check.severity === "pass" ? "" : "!"}</span><p><strong>{check.label}</strong><small>{check.fileName ? `${check.fileName}: ` : ""}{check.detail}</small></p></div>)}
                          </div>
                        </section>
                        <section>
      ...
      {paginationConfirmation ? <div className="confirmation-backdrop" role="presentation"><section className="confirmation-dialog" role="dialog" aria-modal="true" aria-labelledby="numbering-confirmation-title"><p className="eyebrow">Numbering warning</p><h2 id="numbering-confirmation-title">Printed page numbers will differ from PDF page numbers</h2><p className="confirmation-example">{numberingDifferenceExample(paginationConfirmation)}</p><p>The index and witness-statement suggestions will use the configured printed numbering. This is a deliberate legal-numbering choice and does not indicate PDF corruption.</p><div className="download-row"><button className="primary-button" type="button" onClick={acceptPaginationChange}>Use this numbering</button><button className="secondary-button" type="button" onClick={cancelPaginationChange}>Cancel</button></div></section></div> : null}
