...
export function restoreCitedCandidateDecision(
  ...
  return {
    ...
    confirmed: Boolean(evidenceId && saved.confirmed && statementHashMatches),
    confirmationMethod: evidenceId && saved.confirmed && statementHashMatches ? saved.confirmationMethod : undefined,
    confirmedAt: evidenceId && saved.confirmed && statementHashMatches ? saved.confirmedAt : undefined,
    pageStart: saved.pageStart,
    pageEnd: saved.pageEnd,
    sequenceOrder: saved.sequenceOrder,
    repeatDecision: statementHashMatches ? saved.repeatDecision : undefined,
