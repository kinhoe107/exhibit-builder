...
export type BundleLayoutSettings = {
  ...
  /**
   * Retained for saved projects. Optional divider and exhibit-cover pages
   * always count, because the finished PDF uses one page sequence.
   */
...
/**
 * Finished-PDF page order is the only numbering source, so optional divider
 * and exhibit-cover pages always count in stamps, the index and suggestions.
 * The stored layout flag is retained for saved projects but cannot opt out.
 */
export function countsOptionalPagesInReferences(
  _pagination?: Pick<PageNumberSettings, "matchPdfPageOrder">,
  _layout?: Pick<BundleLayoutSettings, "countOptionalPagesInReferences">,
) {
  return true;
}
...
/** Saved projects cannot restart exhibit numbering independently of the finished PDF. */
export function lockPagination(settings: Partial<PageNumberSettings> = {}): PageNumberSettings {
  return {
    ...DEFAULT_PAGINATION,
    ...settings,
    matchPdfPageOrder: true,
