...
export function createBuildPlan(
  ...
  const packedPageCounts = packed.map((volumeItems) => ({
    ...
    referencePages: volumeItems.reduce((total, item) => total + item.contentPages + (options.countOptionalPagesInReferences ? item.optionalPages : 0), 0),
  }));
  const volumes = packed.map((volumeItems, volumeIndex): PlannedBuildVolume => {
    ...
    const plannedItems = volumeItems.map((item): PlannedBuildItem => {
      const countedOptionalPages = options.countOptionalPagesInReferences ? item.optionalPages : 0;
      ...
      const legalStartPage = options.matchPdfPageOrder
      ...
      if (!options.matchPdfPageOrder) referenceCursor = legalEndPage + 1;
  ...
  return deepFreeze({
    ...
    countOptionalPagesInReferences: options.countOptionalPagesInReferences,
    indexNodes,
    multiVolume,
    volumes,
  });
