...
import type { BuildResolution, BundleLayoutSettings, NonA4PageHandling, PageNumberSettings, ProjectSnapshot, ProjectSource, StoredTemplateReview, TemplateDiscrepancyConfirmation, TemplateSlot } from "./bundle-types.ts";
...
export type RecoveryProjectPayload = {
  project?: { name?: string };
  candidates?: Array<Partial<ExhibitCandidate> & { id: string; sourceSha256?: string | null }>;
  ...
  statements?: Array<{ id: string; witnessName: string; witnessInitials: string; sourceId: string }>;
...
export function mergeRecoveryProjectDeltas(opened: OpenedProject, payload: RecoveryProjectPayload, recoveredSources: ProjectSource[] = []) {
  ...
  const candidates = baselineCandidates.map((candidate) => {
    ...
    if (saved.sourceSha256) validCandidateDeltas.add(candidate.id);
    return { ...candidate, ...decision, confirmed: saved.sourceSha256 ? Boolean(decision.confirmed) : false } as ExhibitCandidate;
  });
  ...
  for (const saved of payload.candidates ?? []) {
    ...
    const { sourceSha256: _sourceSha256, ...decision } = saved;
    candidates.push(decision as ExhibitCandidate);
  ...
  return {
    opened: {
      ...
      snapshot: {
        ...
        pagination: { ...opened.snapshot.pagination, ...(payload.pagination ?? {}) },
