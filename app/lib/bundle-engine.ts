...
import { BUNDLE_PROFILES, DEFAULT_BUNDLE_LAYOUT, coverPrintsPageNumber, coverPrintsVolumeLabel, coverWritesMatterText, lockPagination } from "./bundle-types.ts";
...
import {
  ...
  deriveExhibitGroups,
  LOW_CONFIDENCE_SOURCE_QUALIFIER,
  orderExhibitGroups,
  reconcileExhibitArrangement,
  statementReferenceFromCandidate,
  type ReviewProvenance,
} from "./exhibit-groups.ts";
...
import { EMAIL_CHILD_DOCX_ANALYSIS_FAILURE, EMAIL_CHILD_PDF_ANALYSIS_FAILURE, rederiveEmailChildren, type EmailAttachmentChild, type EmailChildDisposition, emailChildrenForDisposition } from "./email-attachments.ts";
...
export type EvidenceRecord = {
  ...
  marker: string | null;
  sha256: string;
  /** Set only by local analysis; build rechecks these reviewed source bytes. */
  verifiedAtAnalysis?: true;
  pageCount: number;
...
export type AnalysisResult = {
  ...
  generatedAt: string;
  statementId?: string;
  /** Exact statement identities and hashes captured from the immutable analysis read. */
  statementSources: Array<{ statementId: string; fileName: string; sha256: string }>;
  /** Immutable analysis-time statement copies available to the authoritative build gate. */
  statementSnapshots: BundleStatementInput[];
  /** Original live statement handles, re-read at save/build to detect replacement. */
  statementHandles: BundleStatementInput[];
  witnessName?: string;
  witnessInitials?: string;
};
...
export type BuildOptions = {
  ...
  /** Cooperative local cancellation, checked between documents and output passes. */
  isCancelled?: () => boolean;
  /** Current statement handles; verified against their analysis-time hashes before assembly. */
  statements?: BundleStatementInput[];
};

const BUILD_CANCELLED = "Bundle build cancelled. No finished bundle was replaced.";

function throwIfBuildCancelled(isCancelled?: () => boolean) {
...
async function createVolumeArchive(
  ...
  throwIfBuildCancelled(isCancelled);
  const zip = new JSZip();
...
export async function finalizeBuildAudit(
  ...
  if (!result.volumes?.length) {
    const finalized = { ...result, manifest: { ...result.manifest, ...audit } };
    throwIfBuildCancelled(isCancelled);
    return finalized;
  }
  ...
  throwIfBuildCancelled(isCancelled);
  const archive = await createVolumeArchive(volumes, result.buildPlan?.bundleIdentity ?? "Exhibit Bundle", isCancelled);
  throwIfBuildCancelled(isCancelled);
...
export const SAMPLE_STATEMENT_FOLDER = "Witness statement";
export const SAMPLE_EXHIBITS_FOLDER = "Exhibits";
export const SAMPLE_UNUSED = "06_SAMPLE_Unreferenced_Checklist.pdf";
...
export function samplePackRelativePath(name: string) {
  if (name === SAMPLE_STATEMENT) return `${SAMPLE_STATEMENT_FOLDER}/${name}`;
  if (name === SAMPLE_UNUSED) return name;
  if (SAMPLE_EVIDENCE.includes(name)) return `${SAMPLE_EXHIBITS_FOLDER}/${name}`;
  return name;
}

function samplePackPublicUrl(name: string) {
  return `/guided-sample/${samplePackRelativePath(name).split("/").map(encodeURIComponent).join("/")}`;
}
...
/** Read one immutable in-memory copy and prove it is the exact analysed file. */
async function verifiedFileSnapshot(file: File, expectedSha256: string, label: string, nextStep = "building") {
  const bytes = await file.arrayBuffer();
  const actualSha256 = await sha256(bytes);
  if (actualSha256 !== expectedSha256) {
    throw new Error(`${label} changed after it was selected. Re-analyse and reconfirm the exact source before ${nextStep}.`);
  }
  return new File([bytes], file.name, { type: file.type, lastModified: file.lastModified });
}

function parentEmailHashesForIncludedExhibits(requested: ExhibitCandidate[], evidence: EvidenceRecord[]) {
  const hashes = new Set<string>();
  const byId = new Map(evidence.map((record) => [record.id, record]));
  for (const candidate of requested) {
    if (candidate.parentEmailProvenance?.parentSha256) hashes.add(candidate.parentEmailProvenance.parentSha256);
    const record = candidate.evidenceId ? byId.get(candidate.evidenceId) : undefined;
    if (record?.derivedFromEmail?.parentSha256) hashes.add(record.derivedFromEmail.parentSha256);
  }
  return hashes;
}

export function mergeRederivedEmailChildren(previous: EmailAttachmentChild[] | undefined, rederived: EmailAttachmentChild[]) {
  const previousByIdentity = new Map((previous ?? []).map((child) => [child.identity, child]));
  return rederived.map((child) => {
    const analysed = previousByIdentity.get(child.identity);
    if (!analysed || analysed.sha256 !== child.sha256) return child;
    return {
      ...child,
      ocrPages: analysed.ocrPages,
      ocrStatus: analysed.ocrStatus,
      ocrFailureReason: analysed.ocrFailureReason,
      pageSizes: analysed.pageSizes,
      pageSizeMeasurementFailed: analysed.pageSizeMeasurementFailed,
      encrypted: analysed.encrypted,
      unsafePdfActions: analysed.unsafePdfActions,
      pdfAnalysisFailure: analysed.pdfAnalysisFailure,
      docxAnalysisFailure: analysed.docxAnalysisFailure,
      workbook: analysed.workbook,
      sheetSelections: analysed.sheetSelections,
    };
  });
}

/**
 * Re-read live selected files, and any parent EML that still authenticates an
 * included standalone attachment, before the compositor uses in-memory copies.
 */
async function verifiedEvidenceSnapshots(
  evidence: EvidenceRecord[],
  selectedEvidenceIds: Set<string>,
  parentHashes: Set<string>,
) {
  const verified = await Promise.all(evidence.map(async (record) => {
    if (record.derivedFromEmail) return record;
    const selected = selectedEvidenceIds.has(record.id);
    const neededAsParent = parentHashes.has(record.sha256);
    if (!selected && !neededAsParent) return record;
    if (!record.verifiedAtAnalysis && !neededAsParent) return record;
    const file = await verifiedFileSnapshot(record.file, record.sha256, record.name);
    if (!neededAsParent && !record.emailAttachments?.length) return { ...record, file };
    const emailAttachments = mergeRederivedEmailChildren(record.emailAttachments, await rederiveEmailChildren(file, record.sha256));
    return { ...record, file, emailAttachments };
  }));
  for (const hash of parentHashes) {
    if (!verified.some((record) => !record.derivedFromEmail && record.sha256 === hash)) {
      throw new Error("A parent email changed after it was selected. Re-analyse and reconfirm the exact source before building.");
    }
  }
  return Promise.all(verified.map(async (record) => {
    if (!record.derivedFromEmail || !selectedEvidenceIds.has(record.id) || !record.verifiedAtAnalysis) return record;
    const parent = verified.find((item) => !item.derivedFromEmail && item.sha256 === record.derivedFromEmail?.parentSha256);
    const child = parent?.emailAttachments?.find((item) => item.identity === record.derivedFromEmail?.childIdentity);
    if (!child) {
      throw new Error(`${record.name} changed after it was selected. Re-analyse and reconfirm the exact source before building.`);
    }
    return { ...record, file: await verifiedFileSnapshot(child.file, record.sha256, record.name) };
  }));
}

/**
 * Reads each current witness statement exactly once, binds that immutable copy
 * to the SHA-256 captured during analysis, and returns snapshots safe to save.
 * The witness bytes are never rewritten.
 */
export async function verifiedStatementSnapshots(
  analysis: AnalysisResult,
  statements: BundleStatementInput[],
): Promise<BundleStatementInput[]> {
  const analysed = analysis.statementSources;
  if (!analysed.length || analysed.length !== statements.length) {
    throw new Error("The witness statement set no longer matches the completed analysis. Re-analyse before saving or building.");
  }
  const currentById = new Map(statements.map((statement) => [statement.id, statement]));
  return Promise.all(analysed.map(async (source) => {
    const statement = currentById.get(source.statementId);
    if (!statement) throw new Error(`${source.fileName} is no longer the witness statement that was analysed. Re-analyse before saving or building.`);
    const file = await verifiedFileSnapshot(statement.file, source.sha256, source.fileName, "saving or building");
...
function createCandidate(
  ...
  // A nearby paragraph is useful review context, but it is not evidence of
  // this citation's document date. Falling back to that wider context can
  // silently attach a preceding or following document's date to this card
  // and then make the wrong source look like the best automatic match.
  const paragraphDate = extractDate(paragraph.text);
  const date = spanOptions?.bindDateToSpan || tokenCount > 1
    ? directDate
    : directDate !== "Date not stated" ? directDate : paragraphDate;
...
async function toEvidence(file: File, index: number): Promise<EvidenceRecord> {
  // A File selected through the browser bridge can be observed more than once
  // while it is being read. Capture it exactly once, then parse, OCR, derive
  // EML attachments and hash that same immutable byte sequence. Keep the
  // original handle in the record so build/save can later detect replacement.
  const sourceFile = file;
  assertInputFileSize(sourceFile);
  const sourceBytes = await sourceFile.arrayBuffer();
  const hash = await sha256(sourceBytes);
  file = new File([sourceBytes], sourceFile.name, { type: sourceFile.type, lastModified: sourceFile.lastModified });
  let text: string;
  let pageCount = 0;
  ...
  if (parsedEmail) {
    ...
    for (const child of emailAttachments) {
      ...
      if (child.extension === "pdf") {
        try {
          const measured = await toEvidence(child.file, 0);
          child.encrypted = measured.encrypted;
          child.unsafePdfActions = measured.unsafePdfActions;
          if (measured.encrypted) {
            child.ocrStatus = "not-needed";
          } else {
            child.pageSizes = (measured.pageSizes ?? []).map((page) => ({
              page: page.page,
              width: page.width,
              height: page.height,
              orientation: page.orientation,
              isA4: page.isA4,
              wouldAddMarginsOnA4: page.wouldAddMarginsOnA4,
              hasAnnotations: page.hasAnnotations ?? false,
            }));
            child.ocrPages = measured.ocrPages;
            child.ocrStatus = measured.ocrStatus;
            child.ocrFailureReason = measured.ocrFailureReason;
          }
        } catch {
          child.pdfAnalysisFailure = EMAIL_CHILD_PDF_ANALYSIS_FAILURE;
        }
      }
      if (child.extension === "docx") {
        try {
          await toEvidence(child.file, 0);
        } catch {
          child.docxAnalysisFailure = EMAIL_CHILD_DOCX_ANALYSIS_FAILURE;
        }
      }
    }
  }
  ...
  return {
    ...
    extension: extensionOf(sourceFile.name),
    text,
    marker:
...
export async function analyseEvidenceFiles(files: File[], startingIndex = 0, onProgress?: (stage: string, detail?: string) => void, isCancelled?: () => boolean) {
  const records: EvidenceRecord[] = [];
  const usable = files.filter((file) => !/^~(?:\$|%24)/i.test(file.name));
  if (startingIndex + usable.length > INPUT_LIMITS.evidenceFiles) throw new Error(`Choose no more than ${INPUT_LIMITS.evidenceFiles} evidence files in one project, including files already added.`);
  for (const [index, file] of usable.entries()) {
    if (isCancelled?.()) throw new Error("Analysis cancelled. No review was created.");
  ...
  if (startingIndex + expanded > INPUT_LIMITS.evidenceFiles) {
...
export async function analyseFiles(
  ...
  if (isCancelled?.()) throw new Error("Analysis cancelled. No review was created.");
  // Capture one immutable analysis input. Citation extraction and the SHA bind
  // to these same bytes; later save/build checks compare the current handle to
  // this exact snapshot rather than trusting a filename or a second read.
  const statementBytes = await statement.arrayBuffer();
  const statementSnapshot = new File([statementBytes], statement.name, { type: statement.type, lastModified: statement.lastModified });
  const { raw, paragraphs } = await extractStatementParagraphs(statementSnapshot);
  ...
  const evidence = await analyseEvidenceFiles(evidenceFiles, 0, onProgress, isCancelled);
  ...
  const statementId = "analysis-statement";
  return {
    statementName: statement.name,
    statementHash,
    statementId,
    statementSources: [{ statementId, fileName: statement.name, sha256: statementHash }],
    statementSnapshots: [{ id: statementId, file: statementSnapshot, witnessName: statement.name.replace(/\.[^.]+$/, ""), witnessInitials: "EX" }],
    statementHandles: [{ id: statementId, file: statement, witnessName: statement.name.replace(/\.[^.]+$/, ""), witnessInitials: "EX" }],
...
export async function analyseBundleStatements(
  ...
  for (const statement of statements) {
    if (isCancelled?.()) throw new Error("Analysis cancelled. No review was created.");
    const result = await analyseFiles(statement.file, evidenceFiles, onProgress, isCancelled);
  ...
  return {
    ...
    statementSources: [{ statementId: sourceStatement.id, fileName: sourceStatement.file.name, sha256: first.statementHash }],
    statementSnapshots: [{ ...sourceStatement, file: first.statementSnapshots[0].file }],
    statementHandles: [sourceStatement],
...
async function fetchSampleFile(name: string) {
  const response = await fetch(samplePackPublicUrl(name));
...
export async function checkSamplePackAvailability() {
  try {
    const responses = await Promise.all(SAMPLE_REQUIRED_FILES.map((name) => fetch(samplePackPublicUrl(name), {
      method: "HEAD",
      cache: "no-store",
    })));
...
export const UNCITED_EXHIBITS_HEADING = "Uncited exhibits — no statement reference";
export const STATEMENT_SUGGESTIONS_READY_HEADING = "Ready to copy";
export const STATEMENT_SUGGESTIONS_VERIFY_HEADING = "Verify the selected source before copying";
...
export function buildStatementUpdateSuggestions(records: BundleRecord[]): StatementUpdateSuggestion[] {
  ...
  const cited = records.flatMap((record) => {
    return record.statementReferences.map((reference) => {
      ...
      const sourceRelativeRange = Number.isSafeInteger(reference.exhibitPageStart) && Number.isSafeInteger(reference.exhibitPageEnd) && Number.isSafeInteger(record.exhibitPageStart)
        ? formatExhibitPageReference(
          reference.exhibitPageStart! - record.exhibitPageStart! + 1,
          reference.exhibitPageEnd! - record.exhibitPageStart! + 1,
        )
        : numericPageRange;
      const requestedConflict = Boolean(requestedRange && sourceRelativeRange && requestedRange !== sourceRelativeRange);
      const lowConfidence = Boolean(
        reference.reviewProvenance
        && reference.reviewProvenance.confidence < AUTOMATIC_MATCH_REVIEW_THRESHOLD,
      );
      const structuralReview = requestedConflict || !pageRange || !sequence || !initials || !validParagraph;
      const needsReview = structuralReview || lowConfidence;
      const citation = pageRange ? `[${exhibit}/${volumePrefix}${pageRange}]` : `[${exhibit}/xx]`;
      const prefix = validParagraph ? `Paragraph ${reference.paragraph}` : "Statement paragraph";
      let line = requestedConflict
        ? `${prefix} - ${citation} (requested ${requestedRange}; review)`
        : structuralReview && !pageRange
          ? `${prefix} - ${citation}`
          : `${prefix} - ${citation}${structuralReview && !requestedConflict ? " (review)" : ""}`;
      if (lowConfidence) line = `${line} (${LOW_CONFIDENCE_SOURCE_QUALIFIER})`;
  ...
  return [
    ...cited,
    { paragraph: null, exhibit: "", pageRange: null, line: UNCITED_EXHIBITS_HEADING, needsReview: false },
    ...uncited,
  ];
}

export function partitionStatementUpdateSuggestions(suggestions: StatementUpdateSuggestion[]) {
  const uncitedIndex = suggestions.findIndex((item) => item.line === UNCITED_EXHIBITS_HEADING);
  const cited = uncitedIndex === -1 ? suggestions : suggestions.slice(0, uncitedIndex);
  return {
    ready: cited.filter((item) => !item.needsReview),
    verify: cited.filter((item) => item.needsReview),
    uncited: uncitedIndex === -1 ? [] : suggestions.slice(uncitedIndex),
  };
}

export function formatStatementUpdateSuggestionText(suggestions: StatementUpdateSuggestion[]) {
  const { ready, verify, uncited } = partitionStatementUpdateSuggestions(suggestions);
  const sections: string[] = [];
  if (ready.length) sections.push(STATEMENT_SUGGESTIONS_READY_HEADING, ...ready.map((item) => item.line));
  if (verify.length) {
    if (sections.length) sections.push("");
    sections.push(STATEMENT_SUGGESTIONS_VERIFY_HEADING, ...verify.map((item) => item.line));
  }
  if (uncited.length) {
    if (sections.length) sections.push("");
    sections.push(...uncited.map((item) => item.line));
  }
...
async function appendEmailChildEvidence(
  ...
  if (child.extension === "pdf") {
    await appendPdfEvidence(output, {
      ...
      rotationPages: [],
      ocrPages: child.ocrPages ?? [],
      ocrStatus: child.ocrStatus ?? "not-needed",
      ocrFailureReason: child.ocrFailureReason,
    }, undefined, undefined, !shouldSkipOcr(child.identity, child.sha256, resolutions), options.pageSizeChoices?.[child.identity] ?? "convert-to-a4");
    return;
  }
  if (child.extension === "xlsx") {
  ...
  if (child.extension === "docx") {
    ...
    try {
      ...
      throw new Error(`${child.name} could not be analysed. Leave the attachment out, or replace the parent email with a readable Word file.`);
    }
...
async function appendPdfEvidence(
  ...
  return appendPdfDocument(output, source, includeGeneratedOcr ? record.ocrPages : [], pageStart, pageEnd, pageSizeHandling);
}
...
async function appendXlsxEvidence(
  ...
  if (selected.some((sheet) => !Array.isArray(sheet.renderPlan.geometryChecks) || !sheet.renderPlan.pageMargins || !Number.isFinite(sheet.renderPlan.printableWidthPoints) || !Number.isFinite(sheet.renderPlan.printableHeightPoints))) {
    throw new Error(`${record.name} uses an older worksheet print analysis without native geometry safeguards. Reanalyse the workbook before building.`);
  }
  // Send one print job per approved worksheet. The analysed plan supplies the
  // approved 85–100% scale and authoritative page breaks; native Excel must
  // reproduce that geometry and page count before any worksheet is appended.
  const requests = selected.map((sheet) => ({
    name: sheet.name,
    range: sheet.renderPlan.range,
    orientation: sheet.renderPlan.orientation,
    scalePercent: sheet.renderPlan.scalePercent,
    columnBreaks: [...new Set(sheet.renderPlan.tiles.map((tile) => tile.left).filter((column) => column > sheet.renderPlan.bounds.left))].sort((left, right) => left - right),
    rowBreaks: [...new Set(sheet.renderPlan.tiles.map((tile) => tile.top).filter((row) => row > sheet.renderPlan.bounds.top))].sort((left, right) => left - right),
    titleRows: sheet.titleRows ?? "",
    titleColumns: "",
    pageOrder: "downThenOver" as const,
    margins: sheet.renderPlan.pageMargins,
    printableWidthPoints: sheet.renderPlan.printableWidthPoints,
    printableHeightPoints: sheet.renderPlan.printableHeightPoints,
    geometryChecks: sheet.renderPlan.geometryChecks,
    expectedPageCount: sheet.renderPlan.predictedPageCount,
  }));
  const exported = await exporter(record.file, requests);
  if (exported.length !== requests.length) throw new Error(`${record.name} did not return every approved worksheet from Microsoft Excel.`);
  const rendered: RenderedSheet[] = [];
  for (const [sheetIndex, sheet] of selected.entries()) {
    const startPage = output.getPageCount() + 1;
    const printed = exported[sheetIndex];
    if (!printed?.bytes?.length || printed.name !== sheet.name) throw new Error(`${record.name} / ${sheet.name} was not printed by Microsoft Excel in the approved order.`);
    const source = await PDFDocument.load(printed.bytes);
    if (source.getPageCount() !== sheet.renderPlan.predictedPageCount) {
      throw new Error(`${record.name} / ${sheet.name} printed ${source.getPageCount()} PDF pages, but the approved workbook plan predicted ${sheet.renderPlan.predictedPageCount}. The worksheet was not added because Excel changed its pagination.`);
    }
...
async function appendPdfDocument(
  ...
  let removedSourceInternalLinks = 0;
  for (const [index, embeddedPage] of embeddedPages.entries()) {
    ...
    if (canCopyDirectly) {
      ...
      removedSourceInternalLinks += removeSourceInternalNavigation(page, output);
      output.addPage(page);
    } else {
  ...
  return removedSourceInternalLinks;
}

/**
 * Source GoTo/Dest links become unsafe after their pages are copied through
 * the body and into the finished volume: their destinations can still point
 * at orphaned page objects. The finished bundle intentionally retains only
 * Exhibit Builder-generated index links and bookmarks.
 */
function removeSourceInternalNavigation(page: PDFPage, document: PDFDocument) {
  // Page-level additional actions can also navigate locally. They have no
  // reliable destination after assembly, so remove inherited page actions.
  page.node.delete(PDFName.of("AA"));
  const annotations = page.node.Annots();
  if (!annotations?.size()) return 0;
  const retained: any[] = [];
  let removed = 0;
  for (let index = 0; index < annotations.size(); index += 1) {
    const reference = annotations.get(index);
    const annotation: any = document.context.lookup(reference);
    const action: any = annotation?.lookup?.(PDFName.of("A"));
    const isInternalNavigation = Boolean(annotation?.get?.(PDFName.of("Dest")))
      || String(action?.get?.(PDFName.of("S"))) === "/GoTo";
    if (isInternalNavigation) {
      removed += 1;
      continue;
    }
    // Annotation /AA is an event-name dictionary. Active non-GoTo actions are
    // rejected before copying; any surviving /AA therefore contains inherited
    // local navigation whose destination is no longer reliable in the bundle.
    // Delete the action dictionary while retaining a non-link annotation's
    // visible appearance and reviewer content.
    if (annotation?.get?.(PDFName.of("AA"))) {
      annotation.delete(PDFName.of("AA"));
      removed += 1;
    }
    retained.push(reference);
  }
  if (removed) page.node.set(PDFName.of("Annots"), document.context.obj(retained));
  return removed;
}
...
function visiblePageLabel(
  ...
  if (settings.matchPdfPageOrder) {
    const startAt = Number.isFinite(settings.startAt) ? Math.max(1, Math.floor(settings.startAt)) : 1;
    const visibleNumber = startAt - 1 + pageNumber + numberOffset;
...
function assertFinishedPdfIsNumberingSourceOfTruth(
  records: BundleRecord[],
  expectedPageLabels: string[],
  indexRecordsById: Map<string, { record: BundleRecord; indexPageLabelStart: string; indexPageLabelEnd: string }>,
) {
  if (indexRecordsById.size !== records.length) {
    throw new Error("Index rows do not match the finished PDF exhibit list.");
  }
  for (const { record, indexPageLabelStart, indexPageLabelEnd } of indexRecordsById.values()) {
    const startLabel = expectedPageLabels[record.startPage - 1] ?? "";
    const endLabel = expectedPageLabels[record.endPage - 1] ?? "";
    if (!startLabel || record.exhibitPageLabelStart !== startLabel || record.exhibitPageLabelEnd !== endLabel) {
      throw new Error(`Finished PDF page labels do not match exhibit ${record.mark} (${record.exhibitPageLabelStart}-${record.exhibitPageLabelEnd} vs ${startLabel}-${endLabel}).`);
    }
    if (indexPageLabelStart !== startLabel || indexPageLabelEnd !== endLabel) {
      throw new Error(`Index page range for ${record.mark} does not match the finished PDF (${indexPageLabelStart}-${indexPageLabelEnd} vs ${startLabel}-${endLabel}).`);
    }
    for (const reference of record.statementReferences) {
      if (!reference.exhibitPageLabelStart) continue;
      if (!expectedPageLabels.includes(reference.exhibitPageLabelStart) || (reference.exhibitPageLabelEnd && !expectedPageLabels.includes(reference.exhibitPageLabelEnd))) {
        throw new Error(`Suggested page labels for ${record.mark} do not appear on the finished PDF.`);
      }
    }
  }
  for (const suggestion of buildStatementUpdateSuggestions(records)) {
    if (!suggestion.pageRange) continue;
    if (!suggestion.line.includes(suggestion.pageRange)) {
      throw new Error(`Statement suggestion "${suggestion.line}" does not print the finished PDF page labels ${suggestion.pageRange}.`);
    }
    const matchesFinishedExhibit = records.some((record) => {
      if (suggestion.paragraph == null) {
        if (!isUncitedRecord(record) || !record.exhibitPageLabelStart || !record.exhibitPageLabelEnd) return false;
        const expectedRange = record.exhibitPageLabelStart === record.exhibitPageLabelEnd
          ? record.exhibitPageLabelStart
          : `${record.exhibitPageLabelStart}-${record.exhibitPageLabelEnd}`;
        return suggestion.pageRange === expectedRange;
      }
      return record.statementReferences.some((reference) => {
        if (suggestion.paragraph != null && reference.paragraph !== suggestion.paragraph) return false;
        if (!reference.exhibitPageLabelStart || !reference.exhibitPageLabelEnd) return false;
        const expectedRange = reference.exhibitPageLabelStart === reference.exhibitPageLabelEnd
          ? reference.exhibitPageLabelStart
          : `${reference.exhibitPageLabelStart}-${reference.exhibitPageLabelEnd}`;
        return suggestion.pageRange === expectedRange;
      });
    });
    if (!matchesFinishedExhibit) {
      throw new Error(`Statement suggestion "${suggestion.line}" does not use the finished PDF page labels for its exhibit.`);
    }
  }
}
...
function addIndexLink(
  ...
  const annotation = document.context.obj({
    Type: "Annot",
    Subtype: "Link",
    NM: PDFHexString.fromText("ExhibitBuilder:index-link"),
...
const GENERATED_INDEX_LINK_NAME = "ExhibitBuilder:index-link";

/**
 * Finished-PDF defence in depth. Source annotations may retain harmless
 * appearances and reviewer comments, but no inherited navigation survives.
 * The sole action annotation permitted in a finished volume is an index link
 * created above and explicitly marked by Exhibit Builder.
 */
function validateFinalAnnotationNavigation(
  document: PDFDocument,
  indexStart: number,
  indexCount: number,
) {
  const indexEnd = indexStart + indexCount - 1;
  let generatedIndexLinks = 0;
  for (const [pageIndex, page] of document.getPages().entries()) {
    const pageNumber = pageIndex + 1;
    if (page.node.get(PDFName.of("AA"))) {
      throw new Error(`Output validation failed: page ${pageNumber} retains an inherited additional action.`);
    }
    const annotations: any = page.node.Annots();
    for (let annotationIndex = 0; annotations && annotationIndex < annotations.size(); annotationIndex += 1) {
      const annotation: any = document.context.lookup(annotations.get(annotationIndex));
      if (!annotation?.get) continue;
      if (annotation.get(PDFName.of("Dest"))) {
        throw new Error(`Output validation failed: page ${pageNumber} annotation ${annotationIndex + 1} retains an inherited destination.`);
      }
      if (annotation.get(PDFName.of("AA"))) {
        throw new Error(`Output validation failed: page ${pageNumber} annotation ${annotationIndex + 1} retains inherited additional actions.`);
      }
      const action: any = annotation.lookup?.(PDFName.of("A"));
      if (!action) continue;
      const subtype = String(action.get?.(PDFName.of("S")));
      const name: any = annotation.get(PDFName.of("NM"));
      const generated = name?.decodeText?.() === GENERATED_INDEX_LINK_NAME;
      if (subtype !== "/GoTo" || !generated || pageNumber < indexStart || pageNumber > indexEnd) {
        throw new Error(`Output validation failed: page ${pageNumber} annotation ${annotationIndex + 1} contains an unapproved inherited action.`);
      }
      generatedIndexLinks += 1;
    }
  }
  return generatedIndexLinks;
}
...
export async function buildBundle(
  ...
  const assertNotCancelled = () => {
    throwIfBuildCancelled(options.isCancelled);
  };
  ...
  // The statement is not added to the exhibit bundle, but its exact analysed
  // identity controls the citations and therefore must always be reverified.
  // A live source handle is mandatory. Immutable analysis snapshots prove
  // what was parsed, but cannot detect that the user's selected file changed.
  const currentStatements = options.statements ?? analysis.statementHandles;
  if (!currentStatements?.length) throw new Error("The analysed witness statement is required before building. Re-analyse the statement and try again.");
  await verifiedStatementSnapshots(analysis, currentStatements);
  assertNotCancelled();
  const profile = BUNDLE_PROFILES.find((item) => item.id === options.profileId) ?? BUNDLE_PROFILES[0];
  const pagination = lockPagination(options.pagination);
  const requestedLayout = { ...DEFAULT_BUNDLE_LAYOUT, ...options.layout };
  // Finished-PDF page order is the only numbering source, so optional pages
  // always belong to that same sequence.
  const layout = {
    ...requestedLayout,
    countOptionalPagesInReferences: true,
  };
  ...
  // Build only from immutable byte snapshots that still match the reviewed
  // SHA-256 values. The original path/File object is never read again by the
  // compositor, so a replacement between review and rendering cannot enter
  // the bundle under an earlier confirmation.
  const selectedEvidenceIds = new Set(requested.map((candidate) => candidate.evidenceId).filter((id): id is string => Boolean(id)));
  const verifiedEvidence = await verifiedEvidenceSnapshots(
    analysis.evidence,
    selectedEvidenceIds,
    parentEmailHashesForIncludedExhibits(requested, analysis.evidence),
  );
  assertNotCancelled();
  analysis = { ...analysis, evidence: verifiedEvidence };
  const derivedGroups = deriveExhibitGroups(analysis, effectiveCandidates);
  const arrangement = reconcileExhibitArrangement(
  ...
  let activeTemplates = (options.templates ?? []).filter((template) => {
  ...
  activeTemplates = await Promise.all(activeTemplates.map(async (template) => {
    if (!usedTemplateSlots.has(template.slot)) return template;
    const file = await verifiedFileSnapshot(template.file, template.sha256, template.file.name);
    const pdfFile = template.pdfFile && template.pdfSha256
      ? await verifiedFileSnapshot(template.pdfFile, template.pdfSha256, `${template.file.name} converted PDF`)
      : template.pdfFile;
    return { ...template, file, pdfFile };
  }));
  assertNotCancelled();
  ...
  for (const [groupIndex, group] of groups.entries()) {
    ...
    if (segmentItems.length > 1 && segmentItems.every((item) => item.evidence.extension === "pdf")) {
      ...
      for (const segment of segmentItems) {
        ...
        refs.push(statementReferenceFromCandidate(segment.member, {
          exhibitInitials: segment.member.exhibitInitials ?? candidate.exhibitInitials ?? candidate.witnessInitials,
          exhibitSequence: segment.member.exhibitSequence ?? candidate.exhibitSequence ?? 1,
          citationResolution: segment.member.citationResolution ?? "none",
          exhibitPageStart: start,
          exhibitPageEnd: end,
        }) as BundleRecord["statementReferences"][number]);
      }
      ...
      const allRefs = group.members.map((member) => {
        ...
        return statementReferenceFromCandidate(member, {
          exhibitInitials: member.exhibitInitials ?? bundleRange.initials,
          exhibitSequence: member.exhibitSequence ?? sequence,
          citationResolution: member.citationResolution ?? "none",
          exhibitPageStart: range ? bundleRange.contentStart + range.start - 1 : undefined,
  ...
  const finalizeVolume = async (volume: PlannedBuildVolume) => {
    ...
    const { numberOffset, preliminaryOffset, firstContentPage, records, recordsById, indexRecordsById } = planned;
    ...
    assertFinishedPdfIsNumberingSourceOfTruth(records, expectedPageLabels, indexRecordsById);
    ...
    const generatedIndexLinks = validateFinalAnnotationNavigation(reopened, coverPageCount + 1, volume.indexPages);
    if (generatedIndexLinks !== expectedIndexLinks.length) {
      throw new Error(`Output validation failed: expected ${expectedIndexLinks.length} marked Exhibit Builder index links, found ${generatedIndexLinks}.`);
    }
    ...
    const checks = [
      ...
      { label: "Source PDF navigation", status: "pass" as const, detail: "Pre-existing internal source-document links were removed during assembly; only Exhibit Builder index links and bookmarks are retained." },
    ...
    const manifest = {
      ...
      pagination,
      layout,
      sourcePdfNavigation: { inheritedInternalLinks: "removed", generatedIndexLinksAndBookmarks: "retained" },
  ...
  const archive = await createVolumeArchive(volumes, buildPlan.bundleIdentity, options.isCancelled);
...
let savePathProtectionReader: () => { protectedSourcePaths: string[]; allowedOverwritePath?: string } = () => ({
  protectedSourcePaths: [],
});

export function setSavePathProtectionReader(
  reader: () => { protectedSourcePaths: string[]; allowedOverwritePath?: string },
): void {
  savePathProtectionReader = reader;
}
...
export function downloadBytes(
  ...
  if (window.bundleBuilderDesktop) {
    const protection = savePathProtectionReader();
    return window.bundleBuilderDesktop.saveFile(bytes, fileName, type, {
      protectedSourcePaths: protection.protectedSourcePaths,
      allowedOverwritePath: protection.allowedOverwritePath,
    });
  }
