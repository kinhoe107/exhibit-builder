...
function boundEmailChildDispositions(
  candidate: Pick<ExhibitCandidate, "emailAttachmentDispositions">,
  evidence: { emailAttachments?: Array<{ identity: string }> } | undefined,
) {
  const allowed = new Set((evidence?.emailAttachments ?? []).map((child) => child.identity));
  const dispositions = candidate.emailAttachmentDispositions ?? {};
  const bound: Record<string, string> = {};
  for (const identity of [...allowed].sort((left, right) => left.localeCompare(right))) {
    const disposition = dispositions[identity];
    if (!disposition) continue;
    bound[identity] = disposition;
  }
  return bound;
}

export function substantiveBuildCanonical(input: BuildFingerprintInput): Record<string, unknown> {
  ...
  const included = input.candidates.filter((candidate) => candidate.included).map((candidate) => {
    ...
    return {
      ...
      emailAttachmentDispositions: boundEmailChildDispositions(candidate, evidence),
...
/** Live retained-PDF checks ignore arrangement/order and build-time outputs. */
export function nonArrangementSubstantiveKey(canonical: Record<string, unknown>): string {
  const { arrangement: _arrangement, order: _order, volumePlan: _volumePlan, statementSuggestions: _statementSuggestions, ...rest } = canonical;
...
export async function createSubstantiveBuildSnapshot(input: BuildFingerprintInput): Promise<SubstantiveBuildSnapshot> {
  const canonical = substantiveBuildCanonical(input);
...
export function compareSubstantiveBuilds(previous: SubstantiveBuildSnapshot | null, current: SubstantiveBuildSnapshot): RebuildDifference {
  ...
  if (exhibitFieldChanged(["emailAttachmentDispositions"])) categories.push("Email attachment decisions changed");
