...
async function loadSafeDocxZip(file: File) {
  ...
  let zip: JSZip;
  try {
    zip = await JSZip.loadAsync(source, { checkCRC32: false, createFolders: false });
  } catch {
...
export async function renderWordTemplateHtml(file: File) {
  ...
  const relationships = relationshipMap(relationshipEntry ? await relationshipEntry.async("text") : "");
...
async function isZipDocument(file: File) {
...
export async function convertWordTemplate(file: File) {
  ...
  if (extension === "doc" && !(await isZipDocument(file))) {
    throw new Error("Legacy .doc templates cannot be rendered faithfully offline. Save the template as .docx or PDF and select it again.");
  }
