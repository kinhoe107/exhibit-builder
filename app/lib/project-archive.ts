...
import { DEFAULT_BUNDLE_LAYOUT, lockPagination, type ProjectSnapshot, type ProjectSource, type StoredTemplateReview, type TemplateDiscrepancyConfirmation } from "./bundle-types.ts";
...
function validMatterGeometry(value: unknown, pageCount: number) {
  ...
  if (typeof pageNumber !== "number" || !Number.isSafeInteger(pageNumber) || pageNumber < 1 || pageNumber > pageCount) return false;
...
export async function createProjectArchive(
  ...
  for (const source of sources) {
    ...
    usedPaths.add(path);
    // Never save bytes under a hash calculated during an earlier analysis.
    // A local file can be replaced between review and project save; bind the
    // portable archive to the exact bytes read for this save or stop safely.
    const bytes = await source.file.arrayBuffer();
    if (await digest(bytes) !== source.sha256) {
      throw new Error(`${source.name} changed after it was selected. Re-analyse and reconfirm the exact source before saving this project.`);
    }
    zip.file(path, bytes);
    storedSources.push({
      id: source.id,
...
export async function openProjectArchive(file: File): Promise<{
  ...
  const { finalOrder: _legacyFinalOrder, ...currentSnapshot } = snapshot;
  // A portable project can prove only that its embedded bytes agree with its
  // own metadata, not who made a human approval. Retain the work but require
  // every confirmation and technical exception to be made again locally.
  const candidates = Array.isArray(snapshot.candidates)
    ? snapshot.candidates.map((candidate) => {
      ...
      delete next.repeatDecision;
      delete next.manualWarningAcknowledgedAt;
      return next;
    })
    : snapshot.candidates;
  const templateReviews = Array.isArray(snapshot.templateReviews)
    ? snapshot.templateReviews.map((review) => ({ ...review, reviewState: { matterReview: review.reviewState.matterReview } }))
    : undefined;
  return {
    snapshot: {
      ...currentSnapshot,
      ...
      schemaVersion: 8,
      arrangement,
      sheetSelections: snapshot.sheetSelections ?? [],
      resolutions: [],
      templateReviews,
      templateDiscrepancyConfirmation: undefined,
      lastBuildSnapshot: undefined,
      pagination: lockPagination(snapshot.pagination ?? {}),
      layout: { ...DEFAULT_BUNDLE_LAYOUT, ...(snapshot.layout ?? {}) },
    } as ProjectSnapshot,
    sources,
  };
}
