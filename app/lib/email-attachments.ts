...
import type { UnsafePdfAction } from "./pdf-action-safety.ts";
...
export type EmailAttachmentChild = {
  ...
  pageSizeMeasurementFailed?: boolean;
  encrypted?: boolean;
  unsafePdfActions?: UnsafePdfAction[];
  /** Bounded user-safe reason when PDF child analysis threw. Separate from page-size metadata. */
  pdfAnalysisFailure?: string;
  /** Bounded user-safe reason when Word child analysis threw. Separate from other child metadata. */
  docxAnalysisFailure?: string;
};

export const EMAIL_CHILD_PDF_ANALYSIS_FAILURE = "The PDF attachment could not be analysed.";
export const EMAIL_CHILD_DOCX_ANALYSIS_FAILURE = "The Word attachment could not be analysed.";
...
async function collectChildren(
  ...
  for (const [index, attachment] of parsed.attachments.entries()) {
    ...
    const file = new File([fileBytes], name, { type: attachment.mimeType || "application/octet-stream" });
    acc.push({
...
export function unresolvedEmailAttachments(
  ...
  return (children ?? []).filter((child) => {
    ...
    if (!child.supported) return !disposition;
    return !disposition;
  });
}
